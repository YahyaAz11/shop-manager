...
class SaleController extends BaseController
{
    ...
    public function index(): void
    {
        ...
        $saleIds = array_map(static fn (array $s): int => (int) ($s['id'] ?? 0), $sales);
        $saleLinesBySaleId = $this->saleItemModel->getGroupedBySaleIds($saleIds);
    ...
    public function store(): void
    {
        ...
        $lineProductIds = array_values(array_unique($lineProductIds));
        ...
        $stockAdjustedLines = [];
        try {
            $vatByProduct = $this->productModel->getVatRatesByProductIds($lineProductIds);

            $saleId = (int) $this->saleModel->create([
                'user_id'         => $userId,
                'total'           => $total,
                'payment_especes' => $pe,
                'payment_carte'   => $pc,
            ...
            if ($saleId < 1) {
                throw new \RuntimeException('Identifiant de vente invalide.');
            }

            foreach ($lines as $line) {
                $pid = (int) $line['product_id'];
                if ($pid < 1) {
                    throw new \InvalidArgumentException('Référence produit invalide.');
                }
                $vatRate = $vatByProduct[$pid] ?? normalize_vat_rate_percent(20);
                ...
                $stockAdjustedLines[] = $line;
            }
        } catch (\Throwable $e) {
            error_log('sale_store: ' . $e->getMessage());
            foreach ($stockAdjustedLines as $ln) {
                try {
                    $this->productModel->increaseStock($ln['product_id'], $ln['quantity']);
                } catch (\Throwable $e2) {
                    error_log('sale_store stock restore: ' . $e2->getMessage());
                }
            }
            if ($saleId > 0) {
                try {
                    $this->saleModel->deleteById($saleId);
                } catch (\Throwable $e2) {
                    error_log('sale_store rollback: ' . $e2->getMessage());
                }
            }
            $_SESSION['flash_error'] = 'Enregistrement impossible. Vérifiez que la base est à jour (colonnes TVA : migrations/001_add_vat_rate.sql). En cas de doute, consultez les logs PHP du serveur.';
            $this->redirect('sale_create');
        }

        $_SESSION['flash_success'] = 'Vente enregistrée.';
        $role = (string) ($this->currentUser()['role'] ?? '');
        $this->redirect($role === 'vendeur' ? 'my_sales' : 'sales');
    }
    ...
    public function show(int $id): void
    {
        ...
        if ($sale) {
            $u = $this->currentUser() ?? [];
            if (($u['role'] ?? '') === 'vendeur'
                && (int) ($sale['user_id'] ?? 0) !== (int) ($u['id'] ?? 0)) {
                $_SESSION['flash_error'] = 'Cette vente ne fait pas partie de vos ventes.';
                $this->redirect('my_sales');
            }
        }
        $items = $sale ? $this->saleItemModel->getBySaleId($id) : [];
